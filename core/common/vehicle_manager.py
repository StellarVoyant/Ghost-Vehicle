...
from opencda.customize.core.actuation.control_manager \
...
from opencda.customize.core.plan.behavior_agent \
    import SpecialAgent
    ...
    def __init__(
        ...
        if 'platooning' in application:
            ...
            self.agent = PlatooningBehaviorAgent(
                ...
                platoon_config,
                carla_map)
        elif 'special' in application:
            self.agent = SpecialAgent(vehicle, carla_map, behavior_config, self.v2x_manager)
        else:
            # todo: new application, like 'special' in application, the below is an example
            self.agent = BehaviorAgent(vehicle, carla_map, behavior_config)

        # Control module
        if 'special' in application:
            self.controller = SpecialControl(control_config, vehicle, self.v2x_manager)
        else:
