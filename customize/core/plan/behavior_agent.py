...
class SpecialAgent(object):
    ...
    def __init__(self, vehicle, carla_map, config_yaml, v2x_manager):
        ...
        for vid, vm in self.v2x_manager.cav_world.get_vehicle_managers().items():
            if vid == self.v2x_manager.vid:
                continue
            self.platoon_list.append(vm.vehicle)
        self.game = GameManager.get_game(self.vehicle, self.platoon_list)
    ...
    def run_step(
        ...
        waypoint = self._map.get_waypoint(self.vehicle.get_location())
        target_loc = waypoint.next(5.0)[0].transform.location
